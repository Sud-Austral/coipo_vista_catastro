...
Your only responsibility is to determine whether the repository's `README.md` accurately describes the repository after the latest changes pushed to `main`.

If the README is outdated and requires a documentation change, update only the necessary parts of `README.md` and create a pull request containing only those documentation changes.

The repository's actual code and configuration are the source of truth.

## Primary objective

Analyze the latest commit pushed to `main`.

Determine whether the changes introduced by that commit make any existing README documentation:

* inaccurate;
* incomplete;
* misleading;
* obsolete;
* or materially outdated.

Do not modify the README merely because code changed.

Only modify it when a meaningful documentation change is justified by evidence in the repository.
...
3. Inspect the files changed by the latest commit.
4. Inspect the relevant source code and configuration files.
5. Inspect existing documentation when relevant.
6. Determine what functionality actually exists in the repository.
7. Compare the documented behavior with the actual implementation.
8. Use the repository's code and configuration as the source of truth.

Do not infer functionality that cannot be verified.

## What should trigger a README update

Consider updating the README when the latest changes affect:
...
* Architecture explicitly documented in the README.
* Project structure explicitly documented in the README.
...
7. Never claim that functionality exists unless it can be verified.
...
13. Do not document unfinished functionality unless the repository clearly identifies it as unfinished.
14. Do not modify files other than `README.md`.
15. Do not modify application source code.
16. Do not modify GitHub Actions workflows.
17. Do not modify dependencies.
18. Do not modify tests.
19. Do not modify configuration files.
...
* Do not change the README's overall structure unless necessary.

## Evidence requirement

Every modification to `README.md` must be supported by evidence from the repository.

Before creating a pull request:

1. Identify exactly which change made the README outdated.
2. Verify the relevant implementation.
3. Verify that the proposed README statement matches the implementation.
4. Review the complete README diff.
5. Remove any unnecessary changes.
6. Ensure that only `README.md` is modified.

If the evidence is insufficient, leave the README unchanged.

## Decision process

After inspecting the latest changes, make one of these decisions internally.
...
Update only the sections required to accurately document the implemented functionality.

Do not rewrite unrelated sections.

## Pull request creation

Create a pull request if and only if:

* `README.md` actually requires changes;
* the changes are supported by repository evidence;
* the resulting diff contains only documentation changes.
...
Do not create more than one pull request during a single workflow execution.
...
* Every new statement in the README is supported by the repository.
* The README diff is minimal and focused.

If any of these conditions cannot be verified, do not create the pull request.
